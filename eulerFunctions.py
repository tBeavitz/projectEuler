...
def getNumOfDivisors(x):

	return len(getFactors(x))
...
def getAdjancyProduct(paramDic):
	grid = paramDic['grid']
...
def getLeastSigDigits(num, digits):

	return int(str(num)[-digits:])

# This Function is not required or particuarly useful in python
def getLSDofSum(x, y, digits):
	x += y
	x = getLeastSigDigits(x, digits)

	return x

# Returns the Longest Collatz Chain that starts with number less than limit
def getLongestCollatz(limit):
	tChain = []
	maxChain = [0]
...
def getUniqueLatticePaths(width, height):
	...
	def processGrid(grid):
		...
		for x,y in it.product(range(width,-1,-1), range(height,-1,-1)):
...
def getAmicableNumbers(limit):
	...
	for x in range(1, limit):
		divs = (getProperDivisors(x))
		amicDic[x] = sum(divs)

	#Check dict i:sum == sum:i
	for i in iter(amicDic):
		if(sum(getProperDivisors(i)) in amicDic):
			if(i != amicDic[i] and i == amicDic[sum(getProperDivisors(i))]):
				amiNum.append(i)
...
# Check if the sum of the proper divisors of x == x
def isPefectNumber(x):
	if(x == sum(getProperDivisors(x))):
		return True
	else: return False

# Check if the sum of the proper divisors of x < x
def isDeficientNumber(x):
	if(x > sum(getProperDivisors(x))):
		return True
	else: return False
...
# Returns a List of all the abundant numbers under the limit	
def getAbundantNumRange(limit):
	numbers = []
...
# Return the permNum lexicographoc permutation of charList 
def lexicographicPerm(charList, permNum):
	perms = it.permutations(charList)
	for x in range(0, permNum-1):
		next(perms)
	return next(perms)